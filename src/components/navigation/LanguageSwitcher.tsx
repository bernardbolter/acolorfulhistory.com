...
export default function LanguageSwitcher() {
  ...
  const toggleLanguage = () => {
    ...
    // Handles home page, dynamic routes, future pathnames config — everything
  }

  return (
    <div className="lng-switch-container" onClick={toggleLanguage}>
      <div className="us-flag-container" style={{ opacity: locale === 'en' ? 0.8 : 0.2 }}>
        <USflag />
      </div>

      <svg
        className={locale === 'de' ? 'switch-svg switch-svg-on' : 'switch-svg'}
        viewBox="0 0 36 20"
      >
      ...
      <div className="de-flag-container" style={{ opacity: locale === 'de' ? 0.8 : 0.2 }}>
        <DEflag />
      </div>
    </div>
  )
